...
export function getSchema(_schema, stateStore = {}) {
  ...
  console.log(_schema, stateStore,'_schema, stateStore_schema, stateStore');
...
export function getParentListByKey(key, schema) {
  ...
  if (!parentList) {
    ...
    if (isArray(children) && children[0] && isArray(children[0].list))
...
export function mergeWidgets(widgets) {
  ...
  const filterWidgets = result.filter(
    (w) =>
      !include(
        groups.map((k) => k.key),
        w.key
...
export function getSchemaType(schema, WidgetSchema) {
  const widgetType =
    WidgetSchema && WidgetSchema.type ? WidgetSchema.type : null;
  const schemaType =
    schema && isNotEmptyString(schema.type) ? schema.type : null;
  // 缺省值
  let type = "string";
